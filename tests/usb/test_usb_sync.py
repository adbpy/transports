"""
    ...
    Tests for the :mod:`~adbts.usb.sync` module.
"""
import pytest
...
def test_open_raises_when_no_device_found(mock_context_no_devices):
    """
    Assert that :func:`~adbts.usb.sync.open` raises a :class:`~adbts.exceptions.TransportEndpointNotFound`
...
def test_open_opens_device_handle(mock_context_one_device_valid_endpoints, mock_device):
    """
    Assert that :func:`~adbts.usb.sync.open` will open a device handle on the selected device.
    """
    usb.sync.open()
...
def test_open_claims_device_handle(mock_device_with_handle, mock_context_one_device_valid_endpoints, mock_handle):
    """
    Assert that :func:`~adbts.usb.sync.open` will claim the interface of the open device handle for
    the selected device.
    """
    usb.sync.open()
